...
| `raw_microsoft_ads`                 | Bronze             | 2 tables: 1,355 campaign-days, 1 account (live 2026-09-15) | Microsoft Advertising (Bing) cost per (ad account, campaign, day) and the account list. Pre-created by `sql/41_raw_microsoft_ads.sql`. Same PK as Google. `spend` / `conversions_value` are decimal TEXT as sent; staging casts. | dbt only |
