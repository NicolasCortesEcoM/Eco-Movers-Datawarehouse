...
| `ads_google_daily` | **06:00 daily** (n8n, **created inactive 2026-09-14**) | `run_ads.py --platform google_ads`: re-reads the last 30 days for every child account under the Google Ads manager into `raw_google_ads.campaign_daily` (merge), because Google restates recent cost and conversions. Not SmartMoving quota - Google Ads API is free, ~1 call per child account per run, logged to the same ledger. Publish only after the developer token is approved for production and the first manual load reconciles to the cent; until then every run fails with `CLOUD_PROJECT_NOT_APPROVED_FOR_PRODUCTION`. Details: `marketing_ads_integration_guide.md` §2. |
