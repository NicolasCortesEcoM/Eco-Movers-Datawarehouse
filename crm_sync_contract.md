...
| `ads_google_daily` | **06:00 daily** (n8n, live since 2026-09-14) | `run_ads.py --platform google_ads`: re-reads the last 30 days for every child account under the Google Ads manager into `raw_google_ads.campaign_daily` (merge), because Google restates recent cost and conversions. Not SmartMoving quota - Google Ads API is free, ~1 call per child account per run, logged to the same ledger. Access level belongs to the Google Cloud project (Explorer, 2,880 ops/day; a run uses ~2). `pipeline_heartbeat` watches it at 30 h. Details: `marketing_ads_integration_guide.md` §2. |
| `ads_microsoft_daily` | **06:10 daily** (n8n, live since 2026-09-15) | `run_ads.py --platform microsoft_ads`: re-reads the last 30 days for every active Microsoft Advertising account into `raw_microsoft_ads.campaign_daily` (merge). Async report per account (Submit + polls + download, ~4 calls), logged to the same ledger; the OAuth refresh token is renewed on every run and rotated in place in the droplet `.env`. Not SmartMoving quota. Retention is 36 months on Microsoft's side. `pipeline_heartbeat` watches it at 30 h. Details: `marketing_ads_integration_guide.md` §4. |
