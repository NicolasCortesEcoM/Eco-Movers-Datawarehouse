...
def load_env() -> dict[str, str]:
    ...
    STRIPS SURROUNDING QUOTES, and that is not cosmetic. The droplet's .env is
    generated by deploy/sync_droplet.py, which single-quotes every value because the
    n8n SSH commands read it with `set -a; . ./.env` and an unquoted password
    containing a space or a `!` is executed as shell rather than assigned.

    The shell strips those quotes on `source`. This parser did not, so dlt received
    the username `'platform_rw'` - quotes included - and every extraction died with
    `password authentication failed for user "'platform_rw'"`. dbt was unaffected
    because it reads the shell environment, so the failure was invisible from the
    dbt side: five days of no API calls while report ingestion looked perfectly
    healthy.

    Only a MATCHED pair is stripped, so a value that legitimately contains a quote
    is left alone.
    """
    env = {}
